...
Sonnar qube merupakan software untuk memeriksa coverage dari unit testing, bug dan duplikasi code dari suatu aplikasi Golang. Untuk bisa pass di Sonar Quba coverage unit testing perlu di atas 80 % dan duplikasi harus dibawah 8%.

## Installasi Sonar Scanner

Sonar scanner digunakan untuk bisa menjalankan command sonar-scanner.bat di terminal bash (git bash) untuk Windows OS.
...
2. Ekstak file ZIP tersebut dan letakkan pada suatu folder.
3. Tambah file tersebut pada path di system variabel di system environment variabel. Untuk Windows OS dapat dengan mengikuti cara berikut.
...
4. Cek dengan comman berikut untuk memastikan sonar-scanner sudah terinstall.
...
Jika menggunakan docker dektop, bisa langsung search sonar qube. Setelah itu klik pull, setelah proses instalasi selesai, sonar qube akan otomatis ada di tab image docker dekstop.

<figure><img src="../.gitbook/assets/sonar qube (1).png" alt=""><figcaption></figcaption></figure>

## Menjalankan Sonar Qube Image

Setelah command tersebut selesai di jalankan, image sonarqube akan muncul di tab images dokcer dekstop.
...
Setelah itu, cek localhost:9000 maka akan muncul tampilan berikut. Masukkan username dan password. Untuk pertama kali login user dan passwordnya adalah admin. Selanjutnya akan diarahkan ke halaman project.
...
7. Scroll ke bawah dan klik bagian angka di samping tanda %, maka akan diarahkan ke hamalan measurement coverage test masing-masing file yang mirip seperti tampilan result.html.
